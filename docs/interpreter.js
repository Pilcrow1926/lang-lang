...
const LangInterpreter = (() => {
  ...
  function parseSingleNumber(text) {
    ...
    if (!(baseChar in BASE_VALUES))
      throw new Error(`잘못된 기본값: ${baseChar}`);
    if ([...text].some((c) => c !== baseChar))
      throw new Error(`혼합된 기본값: ${text}`);
  ...
  function parseDecimalDigits(text) {
    ...
    while (i < text.length) {
      ...
      if (!(baseChar in BASE_VALUES))
        throw new Error(`잘못된 소수 기본값: ${baseChar}`);
      let count = 0;
      while (i < text.length && text[i] === baseChar) {
        count++;
        i++;
      }
  ...
  function parseNumber(text) {
    ...
    if (!text.endsWith("랑"))
      throw new Error(`'랑'으로 끝나야 합니다: ${text}`);
    text = text.slice(0, -1);
  ...
  const SORTED_KEYWORDS = Object.entries(KEYWORDS).sort(
  ...
  function tokenize(source) {
    ...
    while (pos < source.length) {
      ...
      // 수 표현 감지
      const isNum =
        source.slice(pos, pos + 2) === "느그" ||
        (c === "대" &&
          pos + 1 < source.length &&
          "여두근훌".includes(source[pos + 1])) ||
        "여두근훌".includes(c);
  ...
  class Parser {
    ...
    parseStatement() {
      ...
      if (t === "IDENTIFIER") {
        ...
        if (this.peek(1)?.[0] === "INC" || this.peek(1)?.[0] === "DEC")
    ...
    parseAddSub() {
      ...
      while (
        this.current() &&
        (this.current()[0] === "PLUS" || this.current()[0] === "MINUS")
      ) {
    ...
    parseMulDiv() {
      ...
      while (
        this.current() &&
        (this.current()[0] === "MUL" || this.current()[0] === "DIV")
      ) {
  ...
  class Interpreter {
    ...
    evalExpr(expr) {
      ...
      if (t === "Identifier") {
        if (!(expr[1] in this.env))
          throw new Error(`정의되지 않은 변수: ${expr[1]}`);
        return this.env[expr[1]];
      }

      if (t === "BinOp") {
        const [, op, l, r] = expr;
        const lv = this.evalExpr(l),
          rv = this.evalExpr(r);
        if (op === "+") {
          if (typeof lv === "string" || typeof rv === "string")
            return fmt(lv) + fmt(rv);
          return lv + rv;
        }
        if (op === "-") return lv - rv;
        if (op === "*") return lv * rv;
